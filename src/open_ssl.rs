...
impl CTRstream {
    ...
    pub fn encrypt(&mut self, input: Bytes) -> Bytes {
        ...
        for i in 0..input.len() {
            if self.current.is_empty() {
                ...
                for b in encrypt_ecb(data, self.key.clone()).to_bytes() {
...
fn inv_mix_col(col: &mut [u8]) {
    let tmp = [col[0], col[1], col[2], col[3]];
    col[0] = mul(0x0e, tmp[0]) ^ mul(0x0b, tmp[1]) ^ mul(0x0d, tmp[0]) ^ mul(0x09, tmp[1]);
    col[1] = mul(0x09, tmp[0]) ^ mul(0x0e, tmp[1]) ^ mul(0x0b, tmp[0]) ^ mul(0x0d, tmp[1]);
    col[2] = mul(0x0d, tmp[0]) ^ mul(0x09, tmp[1]) ^ mul(0x0e, tmp[0]) ^ mul(0x0b, tmp[1]);
    col[3] = mul(0x0b, tmp[0]) ^ mul(0x0d, tmp[1]) ^ mul(0x09, tmp[0]) ^ mul(0x0e, tmp[1]);
}
