...
impl OracleSimple {
    pub fn new() -> Self {
        Self {key: Bytes::rand(BLOCK_SIZE)}
    }
}
...
impl ProfileOracle {
    pub fn new() -> Self {
        ProfileOracle {key: Bytes::rand(BLOCK_SIZE)}
    }
    pub fn encode_profile(&self, email : Bytes) -> Bytes {
        encrypt_ecb(self.create_profile(email).pad_pkcs7(BLOCK_SIZE), self.key.clone())
    }
    pub fn create_profile(&self, email : Bytes) -> Bytes {
        Bytes::read_utf8("email=") + email.remove('&' as u8).remove('=' as u8) + Bytes::read_utf8("&uid=10&role=user")
    }
    pub fn get_role(&self, profile : Bytes) -> Role {
        for p in decrypt_ecb(profile, self.key.clone()).trim_pkcs7().to_utf8().split("&") {
...
impl Oracle for RandomOracle {
    fn encrypt(&self, input : Bytes) -> Bytes {
        encrypt_ecb((self.prefix.clone()+input+self.target.clone()).pad_pkcs7(BLOCK_SIZE), self.key.clone())
    }
    fn decrypt(&self, input : Bytes) {
        println!("{:16?}", decrypt_ecb(input, self.key.clone()));
    }
...
impl std::fmt::Display for RandomOracle {
    fn fmt(&self, f: &mut std::fmt::Formatter) -> std::fmt::Result {
        write!(f, "Prefix: {}, target: {}", self.prefix.len(), self.target.len())
    }
}
...
impl ProfileCBCOracle {
    pub fn new() -> Self {
        Self {key: Bytes::rand(BLOCK_SIZE), iv: Bytes::rand(BLOCK_SIZE)}
    }
    pub fn encode_profile(&self, email : Bytes) -> Bytes {
        encrypt_cbc(self.create_profile(email).pad_pkcs7(BLOCK_SIZE), self.key.clone(), self.iv.clone())
    }
    pub fn create_profile(&self, email : Bytes) -> Bytes {
        Bytes::read_utf8("comment1=cooking%20MCs;userdata=") + email.remove(';' as u8).remove('=' as u8) + Bytes::read_utf8(";comment2=%20like%20a%20pound%20of%20bacon")
    }
    pub fn get_role(&self, profile : Bytes) -> Role {
        for p in decrypt_cbc(profile, self.key.clone(), self.iv.clone()).trim_pkcs7().to_utf8().split(";") {
            let kv : Vec<&str> = p.split("=").collect();
            if kv[0] == "admin" {
                if kv[1] == "false" {
                    return Role::USER;
    ...
    #[allow(dead_code)]
    pub fn print_raw(&self, profile : Bytes) {
        println!("{0:16?}{0:16X}", decrypt_cbc(profile, self.key.clone(), self.iv.clone()));
    }
}
...
fn get_rand() -> Bytes {
    let r = match thread_rng().gen_range(0, 10) {
        ...
        1 => Bytes::read_64("MDAwMDAxV2l0aCB0aGUgYmFzcyBraWNrZWQgaW4gYW5kIHRoZSBWZWdhJ3MgYXJlIHB1bXBpbic="),
